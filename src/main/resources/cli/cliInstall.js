...
function runInstall(runInfo) {
	...
	var briefJson = new File(installDir, "brief.json")
	...
	if (!bitcode) {
		print("Package name was not informed.")
		return
	...
	print("Installing bitcode: " + owner + "/" + repository + "...")

	var libBriefJson = client.getBriefJson(owner, repository)

	if (!libBriefJson) {
		print("Invalid bitcode, 'brief.json' was not found on " + owner + "/" + repository)
		return
	}
	...
	print("Bitcode " + owner + "/" + repository + " installed.")
}
